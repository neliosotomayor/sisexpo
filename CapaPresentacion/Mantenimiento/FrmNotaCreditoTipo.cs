...
{
    public partial class FrmNotaCreditoTipo : Form
    {
        ...
        private static FrmNotaCreditoTipo _instance;
        
        public static FrmNotaCreditoTipo Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new FrmNotaCreditoTipo();
                return _instance;
            }
        }

        public int FKIdNotaCreditoTipo = 0;
        private int iIdNotaCreditoTipo = 0;
        private bool bModifica = false;

        CENotaCreditoTipo objEntidad = new CENotaCreditoTipo();
        CNNotaCreditoTipo objNegocio = new CNNotaCreditoTipo();

        #region Procedimientos

        public void MostrarBuscarTabla(string sBuscar)
        {
            //---------------------CNSeguros objNegocio = new CNSeguros();
            dgvDatos.DataSource = objNegocio.MostrarNotaCreditoTipos(sBuscar);
        }

        public void MostrarColumnas()
        {
            dgvDatos.Columns[0].Visible = false;   //NCTip_Id

            dgvDatos.Columns[1].Visible = true;    //NCTip_Descripcion
            dgvDatos.Columns[1].Width = 100;       //NCTip_Descripcion
        ...
        {
            iIdNotaCreditoTipo = Convert.ToInt32(dgvDatos.CurrentRow.Cells[0].Value);
            txtDescripcion.Text = dgvDatos.CurrentRow.Cells[1].Value.ToString();
        }

        #endregion

        public FrmNotaCreditoTipo()
        {
            InitializeComponent();
        }

        private void FrmNotaCreditoTipo_Load(object sender, EventArgs e)
        {
            LimpiarControles();
            MostrarBuscarTabla("");
            MostrarColumnas();
        ...
        private void FrmNotaCreditoTipo_FormClosed(object sender, FormClosedEventArgs e)
        {
            _instance = null;
            ...
            if (cFuncion.PuedeEliminar())
            {
                ...
                if (result == DialogResult.OK)
                {
                    try
                    {
                        ...
                        objNegocio.EliminarNotaCreditoTipo(objEntidad);
        ...
        {
            //if (FKIdNotaCreditoTipo == 1)
            //{
            //    FrmVehiculos formPadre = Owner as FrmVehiculos;
            //    formPadre.txtSeguroId.Text = iIdNotaCreditoTipo.ToString();
            //    formPadre.lblSeguro.Text = dgvDatos.CurrentRow.Cells[1].Value.ToString();
            //    FKIdNotaCreditoTipo = 0;
            //    this.Close();
            //}
            //if (FKIdNotaCreditoTipo == 2)
            //{
            //    FrmVehiculos formPadre = Owner as FrmVehiculos;
            //    formPadre.txtSOATId.Text = iIdNotaCreditoTipo.ToString();
            //    formPadre.lblSOAT.Text = dgvDatos.CurrentRow.Cells[1].Value.ToString();
            //    FKIdNotaCreditoTipo = 0;
            //    this.Close();
            //}
