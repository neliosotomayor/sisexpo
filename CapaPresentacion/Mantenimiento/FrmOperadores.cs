...
namespace CapaPresentacion
{
    public partial class FrmOperadores : Form
    {
        ...
        private static FrmOperadores _instance;

        public static FrmOperadores Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new FrmOperadores();
                return _instance;
            }
        }

        public int FKIdOperador = 0;
        private int iIdOperador = 0;
        private bool bModifica = false;

        CEOperador objEntidad = new CEOperador();
        CNOperador objNegocio = new CNOperador();

        #region Procedimientos

        public void MostrarBuscarTabla(string sBuscar)
        {
            dgvDatos.DataSource = objNegocio.MostrarOperadores(sBuscar);
        }

        public void MostrarColumnas()
        {
            dgvDatos.Columns[0].Visible = false;  //Ope_Id

            dgvDatos.Columns[1].Visible = true;   //Ope_Nombre
            dgvDatos.Columns[1].Width = 100;      //Ope_Nombre
        ...
        {
            iIdOperador = Convert.ToInt32(dgvDatos.CurrentRow.Cells[0].Value);
        ...
        private void bfbGrabar_Click(object sender, EventArgs e)
        {
            if (bModifica == false)
            {
                if (txtNombre.Text.Length > 0)
                {
                    ...
                    try
                    {
                        objEntidad.Ope_Nombre = txtNombre.Text;
                        objEntidad.Usuario_Creacion = FrmLogin.gUsuario;   //Usuario logueado al sistema

                        objNegocio.InsertarOperador(objEntidad);
            ...
            if (cFuncion.PuedeEliminar())
            {
                ...
                if (result == DialogResult.OK)
                {
                    try
                    {
                        ...
                        objNegocio.EliminarOperador(objEntidad);
